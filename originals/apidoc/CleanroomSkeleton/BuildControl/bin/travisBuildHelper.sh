...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project CleanroomCLI.xcodeproj -configuration Debug -scheme CleanroomCLI-$PLATFORM -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "CleanroomCLI-$PLATFORM-$OPERATION.log" | xcpretty )
